(function () {
    ...
    var BTN_ICON = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30.051 30.051" width="512" height="512">' +
    ...
    function init() {
        ...
        Lampa.Listener.follow('full', function (e) {
            ...
            // Удаляем старую кнопку если уже есть
            render.find('.view--online').remove();

            // Создаём кнопку в том же формате, что .view--torrent и .view--trailer
            var btn = $('<div class="full-start__button selector view--online"></div>');
            btn.append(BTN_ICON);
            btn.append('<span>' + T('online_title') + '</span>');
            ...
            // ── Вставляем в .buttons--container (скрытый контейнер с view-- кнопками) ──
            // Именно отсюда Lampa берёт кнопки в меню "Смотреть"
            var container = render.find('.buttons--container');
            if (container.length) {
                // Вставляем перед .view--trailer, либо в конец
                var trailer = container.find('.view--trailer');
                if (trailer.length) trailer.before(btn);
                else container.append(btn);
            } else {
                // Старые версии Lampa — ищем .view--trailer напрямую
                var trailer2 = render.find('.view--trailer');
                if (trailer2.length) { trailer2.before(btn); }
                else {
                    // Самый крайний fallback — ищем .view--torrent
                    var torrent = render.find('.view--torrent');
                    if (torrent.length) { torrent.after(btn); }
                    else {
                        // Добавляем в конец .full-start-new__buttons
