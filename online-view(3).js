(function () {
    ...
    function OnlineViewComponent(object) {
        ...
        if (available.length > 1) {
            ...
            // Создаём контроллер для навигации по вкладкам
            tabsController = new Lampa.Controller({
                toggle: function () {
                    Lampa.Controller.add('ov_tabs', {
                        toggle: function () {
                            this.render().find('.ov-tab').removeClass('focus');
                        },
                        right: function () {
                        ...
                        up: function () {
                            // При нажатии вверх ничего не делаем
                        },
                        down: function () {
                            // При нажатии вниз фокус на iframe (если нужно)
                        },
                        back: function () {
        ...
        this.start   = function () {
            // Инициализация навигации при запуске компонента
            if (tabsController) {
                Lampa.Controller.add('ov_tabs', {
                    toggle: function () {
                        this.render().find('.ov-tab').removeClass('focus');
                ...
                Lampa.Controller.toggle('ov_tabs');
            }

            // Навигация через селекторы Lampa
            var Navigator = new Lampa.Navigator($root.find('.ov-tab'));
            Navigator.on('select', function (tab, index) {
                self.load(index);
            });
        };

        this.pause   = function () {
            // Убираем контроллер при паузе
            Lampa.Controller.remove('ov_tabs');
        };
        ...
        this.destroy = function () {
            ...
            Lampa.Controller.remove('ov_tabs');
            if (tabsController) tabsController.destroy();
        };
    }
