(function() {
  ...
  var preferredQuality = Lampa.Storage.get('lampac_quality_priority', 'auto'); // auto, 2160p, 1080p, 720p
  ...
  function component(object) {
    ...
    this.setDefaultQuality = function(data) {
      if (Lampa.Arrays.getKeys(data.quality).length) {
        ...
        if (qualityInfo.found.length > 0) {
          console.log('Lampac: Available qualities -', qualityInfo.found.join(', '));
        }
        
        // Применяем выбранное качество
        if (preferredQuality !== 'auto' && data.quality[preferredQuality]) {
          data.url = data.quality[preferredQuality];
          console.log('Lampac: Using preferred quality', preferredQuality);
        } else if (preferredQuality === 'auto') {
          // Автовыбор лучшего качества
          var priority = ['2160p', '1440p', '1080p', '720p', '480p', '360p'];
          for (var i = 0; i < priority.length; i++) {
            var q = priority[i];
            if (data.quality[q]) {
              data.url = data.quality[q];
              console.log('Lampac: Auto-selected quality', q);
              break;
            }
          }
        } else {
          // Стандартная логика Lampa
          ...
          _this5.getFileUrl(item, function(json, json_call) {
            if (json && json.url) {
              ...
              if (first.quality && Object.keys(first.quality).length > 0) {
                ...
                if (qualityInfo.found.length > 0) {
                  console.log('Lampac: Playing with available qualities -', qualityInfo.found.join(', '));
    ...
    this.parse = function(str) {
      try {
        ...
        console.error('Lampac parse error:', e);
    ...
    this.draw = function(items) {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          // Показываем информацию о доступных качествах
    ...
    this.contextMenu = function(params) {
      params.html.on('hover:long', function() {
        function show(extra) {
          ...
          // Добавляем опцию выбора качества
          if (params.element.qualitys && Object.keys(params.element.qualitys).length > 1) {
            ...
            var qualityInfo = this.checkAvailableQualities(params.element.qualitys);
            var priority = ['2160p', '1440p', '1080p', '720p', '480p', '360p'];
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              if (a.quality) {
                Lampa.Storage.set('lampac_quality_priority', a.quality);
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '1.6.4',
      name: 'Lampac',
      description: 'Плагин для просмотра онлайн сериалов и фильмов с поддержкой 4K/1080p',
      component: 'lampac',
      onContextMenu: function onContextMenu(object) {
        return {
          name: Lampa.Lang.translate('lampac_watch'),
          description: 'Поддержка 4K и 1080p'
        };
      },
    ...
    Lampa.Lang.add({
      lampac_watch: {
        ru: 'Смотреть онлайн (4K/1080p)',
        en: 'Watch online (4K/1080p)',
        uk: 'Дивитися онлайн (4K/1080p)',
        zh: '在线观看 (4K/1080p)'
      },
      lampac_video: {
        ru: 'Видео',
    ...
    var button = `<div class="full-start__button selector view--online lampac--button" data-subtitle="${manifst.name} v${manifst.version}">
    ...
    // Добавляем настройку качества в интерфейс Lampa
    function addQualitySettings() {
      if (Lampa.Settings) {
        Lampa.Settings.add({
          component: 'settings_main',
          panel: {
            name: 'lampac_quality_settings',
            title: 'Настройки Lampac',
            items: [
              {
                name: 'lampac_quality_priority',
                component: 'selector',
                title: 'Приоритет качества',
                values: [
                  { title: 'Авто (лучшее доступное)', value: 'auto' },
                  { title: '4K (2160p)', value: '2160p' },
                  { title: '1080p (Full HD)', value: '1080p' },
                  { title: '720p (HD)', value: '720p' },
                  { title: '480p (SD)', value: '480p' }
                ],
                value: Lampa.Storage.get('lampac_quality_priority', 'auto'),
                onChange: function(value) {
                  Lampa.Storage.set('lampac_quality_priority', value);
                  preferredQuality = value;
                  Lampa.Noty.show('Настройки качества сохранены');
                }
              }
            ]
          }
