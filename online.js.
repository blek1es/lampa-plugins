(function() {
  ...
  var preferredQuality = Lampa.Storage.get('rezka_quality_priority', 'auto'); // auto, 2160p, 1440p, 1080p, 720p
  ...
  function component(object) {
    ...
    // Функция проверки доступных качеств
    this.checkAvailableQualities = function(qualities) {
      var available = {
        '2160p': false,
        '1440p': false,
        '1080p': false,
      ...
      for (var q in qualities) {
        var qualityKey = q.toLowerCase();
        
        if (qualityKey.includes('2160') || qualityKey.includes('4k') || qualityKey.includes('uhd') || qualityKey.includes('ultra')) {
          available['2160p'] = true;
          found.push('4K (2160p)');
        } else if (qualityKey.includes('1440') || qualityKey.includes('2k') || qualityKey.includes('qhd')) {
          available['1440p'] = true;
          found.push('2K (1440p)');
        } else if (qualityKey.includes('1080') || qualityKey.includes('fhd') || qualityKey.includes('fullhd')) {
          available['1080p'] = true;
          found.push('1080p (Full HD)');
        } else if (qualityKey.includes('720') || qualityKey.includes('hd')) {
          available['720p'] = true;
          found.push('720p (HD)');
        } else if (qualityKey.includes('480') || qualityKey.includes('sd')) {
          available['480p'] = true;
          found.push('480p (SD)');
        } else if (qualityKey.includes('360')) {
          available['360p'] = true;
          found.push('360p');
        }
      }
      ...
      return {
        available: available,
        found: found,
        best: available['2160p'] ? '2160p' : 
              available['1440p'] ? '1440p' : 
              available['1080p'] ? '1080p' : 
              available['720p'] ? '720p' : 
              available['480p'] ? '480p' : '360p'
      };
    };
    
    // Оптимизированная функция для Rezka
    this.getRezkaQuality = function(qualities) {
      // Приоритет для Rezka: 4K → 1440p → 1080p → 720p
      var priority = ['2160p', '1440p', '1080p', '720p', '480p', '360p'];
      
      if (preferredQuality !== 'auto' && qualities[preferredQuality]) {
        return {
          quality: preferredQuality,
          url: qualities[preferredQuality]
        };
      }
      
      // Автовыбор лучшего доступного качества
      ...
      // Если качеств нет, берем первое доступное
      var firstKey = Object.keys(qualities)[0];
      return {
        quality: firstKey,
        url: qualities[firstKey]
      };
    };
    ...
    this.initialize = function() {
      ...
      filter.render().find('.filter--sort span').text(Lampa.Lang.translate('rezka_balanser'));
      scroll.body().addClass('torrent-list');
      files.appendFiles(scroll.render());
      files.appendHead(filter.render());
      scroll.minus(files.render().find('.explorer__files-head'));
      scroll.body().append(Lampa.Template.get('rezka_content_loading'));
    ...
    this.updateBalanser = function(balanser_name) {
      var last_select_balanser = Lampa.Storage.cache('rezka_last_balanser', 3000, {});
      last_select_balanser[object.movie.id] = balanser_name;
      Lampa.Storage.set('rezka_last_balanser', last_select_balanser);
    };
    
    this.changeBalanser = function(balanser_name) {
      this.updateBalanser(balanser_name);
      Lampa.Storage.set('rezka_balanser', balanser_name);
    ...
    this.getLastChoiceBalanser = function() {
      ...
      if (last_select_balanser[object.movie.id]) {
        ...
        return Lampa.Storage.get('rezka_balanser', filter_sources.length ? filter_sources[0] : '');
      }
    };
      ...
      return new Promise(function(resolve, reject) {
        ...
        if (filter_sources.length) {
          ...
          if (last_select_balanser[object.movie.id]) {
            ...
            balanser = Lampa.Storage.get('rezka_balanser', filter_sources[0]);
          }
          if (!sources[balanser]) balanser = filter_sources[0];
          if (!sources[balanser].show && !object.lampac_custom_select) balanser = filter_sources[0];
          source = sources[balanser].url;
          Lampa.Storage.set('rezka_active_balanser', balanser);
          resolve(json);
        } else {
          reject();
        }
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            if (life_wait_times > 15 || json.ready) {
              filter.render().find('.rezka-balanser-loader').remove();
              gou(json, true);
    ...
    this.createSource = function() {
      var _this4 = this;
      return new Promise(function(resolve, reject) {
        var url = _this4.requestParams(Defined.localhost + 'lite/rezka/events?life=true');
        ...
        network.silent(account(url), function(json) {
          ...
          if (json.life) {
            ...
            filter.render().find('.filter--sort').append('<span class="rezka-balanser-loader" style="width: 1.2em; height: 1.2em; margin-top: 0; background: url(./img/loader.svg) no-repeat 50% 50%; background-size: contain; margin-left: 0.5em"></span>');
    ...
    this.setRezkaQuality = function(data) {
      if (Lampa.Arrays.getKeys(data.quality).length) {
        var qualityInfo = this.checkAvailableQualities(data.quality);
        
        // Показываем доступные качества в логе
        if (qualityInfo.found.length > 0) {
          console.log('Rezka: Available qualities -', qualityInfo.found.join(', '));
          
          // Уведомление о доступном качестве
          if (qualityInfo.available['2160p']) {
            Lampa.Noty.show('Доступно качество 4K!', {time: 3000});
          } else if (qualityInfo.available['1440p']) {
            Lampa.Noty.show('Доступно качество 2K (1440p)', {time: 3000});
          } else if (qualityInfo.available['1080p']) {
            Lampa.Noty.show('Доступно качество 1080p', {time: 2000});
          }
        }
        
        // Получаем лучшее качество для Rezka
        var bestQuality = this.getRezkaQuality(data.quality);
        data.url = bestQuality.url;
        console.log('Rezka: Selected quality', bestQuality.quality);
        
        // Добавляем информацию о качестве в данные
        data.selected_quality = bestQuality.quality;
        data.available_qualities = qualityInfo.found;
      }
    };
        ...
        onEnter: function onEnter(item, html) {
          _this5.getFileUrl(item, function(json, json_call) {
            if (json && json.url) {
              ...
              _this5.setRezkaQuality(first);
              
              // Показываем информацию о качестве
              if (first.quality && Object.keys(first.quality).length > 0) {
                var qualityInfo = _this5.checkAvailableQualities(first.quality);
                if (qualityInfo.found.length > 0) {
                  console.log('Rezka: Playing with available qualities -', qualityInfo.found.join(', '));
                }
              }
                        ...
                        cell.url = function(call) {
                          _this5.getFileUrl(elem, function(stream, stream_json) {
                            if (stream.url) {
                              ...
                              _this5.setRezkaQuality(cell);
                              elem.mark();
                            } else {
                              cell.url = '';
                              Lampa.Noty.show(Lampa.Lang.translate('rezka_nolink'));
                            }
                            call();
                          }, function() {
                            cell.url = '';
            ...
            } else Lampa.Noty.show(Lampa.Lang.translate('rezka_nolink'));
          }, true);
        },
    ...
    this.parse = function(str) {
      try {
        ...
        console.error('Rezka parse error:', e);
        this.doesNotAnswer({accsdb: false});
      }
    };
    ...
    this.similars = function(json) {
      ...
      json.forEach(function(elem) {
        ...
        var item = Lampa.Template.get('rezka_prestige_folder', elem);
        if (elem.img) {
    ...
    this.getChoice = function(for_balanser) {
      var data = Lampa.Storage.cache('rezka_choice_' + (for_balanser || balanser), 3000, {});
    ...
    this.saveChoice = function(choice, for_balanser) {
      ...
      Lampa.Storage.set('rezka_choice_' + (for_balanser || balanser), data);
    ...
    this.watched = function(set) {
      ...
      var watched = Lampa.Storage.cache('rezka_watched_last', 5000, {});
      if (set) {
        if (!watched[file_id]) watched[file_id] = {};
        Lampa.Arrays.extend(watched[file_id], set, true);
        Lampa.Storage.set('rezka_watched_last', watched);
        this.updateWatched();
      } else {
    ...
    this.updateWatched = function() {
      ...
      } else body.append('<span>' + Lampa.Lang.translate('rezka_no_watch_history') + '</span>');
    };
    ...
    this.draw = function(items) {
      ...
      scroll.clear();
      if(!object.balanser)scroll.append(Lampa.Template.get('rezka_prestige_watched', {}));
      this.updateWatched();
      this.getEpisodes(items[0].season, function(episodes) {
        var viewed = Lampa.Storage.cache('rezka_view', 5000, []);
        ...
        items.forEach(function(element, index) {
          // Показываем информацию о доступных качествах для Rezka
          if (element.qualitys && Object.keys(element.qualitys).length > 0) {
            var qualityInfo = _this8.checkAvailableQualities(element.qualitys);
            if (qualityInfo.found.length > 0) {
              // Добавляем информацию о качестве в заголовок
              element.qualityInfo = qualityInfo.found.join(', ');
              
              // Показываем иконку 4K если доступно
              if (qualityInfo.available['2160p']) {
                element.quality = '4K';
                element.qualityClass = 'quality-4k';
              } else if (qualityInfo.available['1440p']) {
                element.quality = '2K';
                element.qualityClass = 'quality-2k';
              } else if (qualityInfo.available['1080p']) {
                element.quality = '1080p';
                element.qualityClass = 'quality-1080p';
              } else if (qualityInfo.available['720p']) {
                element.quality = '720p';
                element.qualityClass = 'quality-720p';
              }
            }
          }
          ...
          var hash_timeline = Lampa.Utils.hash(element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title].join(''));
          ...
          if (episode) {
            ...
            if (element.info.length < 30 && episode.vote_average) info.push(Lampa.Template.get('rezka_prestige_rate', {
          ...
          // Добавляем бейдж качества, если доступен
          if (element.quality) {
            element.info += ' <span class="quality-badge ' + element.qualityClass + '">' + element.quality + '</span>';
          }
          
          var html = Lampa.Template.get('rezka_prestige_full', element);
          ...
          element.mark = function() {
            viewed = Lampa.Storage.cache('rezka_view', 5000, []);
            if (viewed.indexOf(hash_behold) == -1) {
              viewed.push(hash_behold);
              Lampa.Storage.set('rezka_view', viewed);
          ...
          element.unmark = function() {
            ...
            if (viewed.indexOf(hash_behold) !== -1) {
              ...
              Lampa.Storage.set('rezka_view', viewed);
              Lampa.Storage.remove('rezka_view', hash_behold);
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function(episode) {
            var info = [];
            if (episode.vote_average) info.push(Lampa.Template.get('rezka_prestige_rate', {
              rate: parseFloat(episode.vote_average + '').toFixed(1)
            }, true));
            ...
            var html = Lampa.Template.get('rezka_prestige_full', {
              time: Lampa.Utils.secondsToTime((episode ? episode.runtime : object.movie.runtime) * 60, true),
    ...
    this.contextMenu = function(params) {
      ...
      params.html.on('hover:long', function() {
        function show(extra) {
          ...
          // Добавляем опцию выбора качества для Rezka
          if (params.element.qualitys && Object.keys(params.element.qualitys).length > 1) {
            menu.push({
              title: 'Выбор качества (Rezka)',
              separator: true
            });
            
            var qualityInfo = _this.checkAvailableQualities(params.element.qualitys);
            var priority = ['2160p', '1440p', '1080p', '720p', '480p', '360p'];
            
            for (var i = 0; i < priority.length; i++) {
              var q = priority[i];
              if (params.element.qualitys[q]) {
                var qualityName = q === '2160p' ? '4K (2160p)' : 
                                 q === '1440p' ? '2K (1440p)' : 
                                 q === '1080p' ? '1080p (Full HD)' : 
                                 q === '720p' ? '720p (HD)' : 
                                 q === '480p' ? '480p (SD)' : '360p';
          ...
          menu.push({
            title: Lampa.Lang.translate('rezka_video'),
            separator: true
          });
          menu.push({
          ...
          if (window.rezka_online_context_menu)
            window.rezka_online_context_menu.push(menu, extra, params);
          menu.push({
          ...
          if (Lampa.Account.logged() && params.element && typeof params.element.season !== 'undefined' && params.element.translate_voice) {
            menu.push({
              title: Lampa.Lang.translate('rezka_voice_subscribe'),
              subscribe: true
            });
          }
          menu.push({
            title: Lampa.Lang.translate('rezka_clear_all_marks'),
            clearallmark: true
          });
          menu.push({
            title: Lampa.Lang.translate('rezka_clear_all_timecodes'),
            timeclearall: true
          });
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              if (a.quality) {
                Lampa.Storage.set('rezka_quality_priority', a.quality);
                preferredQuality = a.quality;
                Lampa.Noty.show('Качество установлено: ' + 
                  (a.quality === '2160p' ? '4K' : 
                   a.quality === '1440p' ? '2K (1440p)' :
                   a.quality === '1080p' ? '1080p' : a.quality));
              }
              ...
              if (window.rezka_online_context_menu)
                window.rezka_online_context_menu.onSelect(a, params);
              Lampa.Controller.toggle(enabled);
              if (a.player) {
              ...
              if (a.subscribe) {
                Lampa.Account.subscribeToTranslation({
                  ...
                  Lampa.Noty.show(Lampa.Lang.translate('rezka_voice_success'));
                }, function() {
                  Lampa.Noty.show(Lampa.Lang.translate('rezka_voice_error'));
                });
              }
            }
          });
        ...
        if (Lampa.Helper) Lampa.Helper.show('rezka_file', Lampa.Lang.translate('helper_rezka_file'), params.html);
      });
    };
    
    this.empty = function() {
      var html = Lampa.Template.get('rezka_does_not_answer', {});
    ...
    this.noConnectToServer = function(er) {
      ...
      html.find('.online-empty__time').text(er && er.accsdb ? er.msg : Lampa.Lang.translate('rezka_does_not_answer_text').replace('{balanser}', sources[balanser] ? sources[balanser].name : balanser));
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    };
    ...
    this.destroy = function() {
      ...
      clearTimeout(number_of_requests_timer);
      if (window.nwsClient && window.nwsClient[hostkey]) {
        window.nwsClient[hostkey].socket.close();
      }
    };
  }
    ...
    var source = {
      ...
      onSelect: function(params, close) {
        ...
        Lampa.Activity.push({
          ...
          title: 'Rezka - ' + params.element.title,
          component: 'rezka',
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '2.0.0',
      name: 'Rezka Online',
      description: 'Плагин для просмотра онлайн с Rezka.ag (4K, 1440p, 1080p, 720p)',
      component: 'rezka',
      onContextMenu: function onContextMenu(object) {
        return {
          name: Lampa.Lang.translate('rezka_watch'),
          description: 'Поддержка 4K, 1440p, 1080p'
        };
      },
      onContextLauch: function onContextLauch(object) {
        ...
        Lampa.Activity.push({
          url: '',
          title: Lampa.Lang.translate('title_rezka_online'),
          component: 'rezka',
    ...
    addSourceSearch('Rezka', 'rezka');
    
    Lampa.Manifest.plugins = manifst;
    
    // Добавляем переводы для качества
    Lampa.Lang.add({
      rezka_watch: {
        ru: 'Смотреть на Rezka (4K/1440p/1080p)',
        en: 'Watch on Rezka (4K/1440p/1080p)',
        uk: 'Дивитися на Rezka (4K/1440p/1080p)',
        zh: '在 Rezka 上观看 (4K/1440p/1080p)'
      },
      rezka_video: {
        ru: 'Видео Rezka',
        en: 'Rezka Video',
      ...
      rezka_no_watch_history: {
        ru: 'Нет истории просмотра Rezka',
        en: 'No Rezka browsing history',
        ua: 'Немає історії перегляду Rezka',
        zh: '没有 Rezka 浏览历史'
      },
      rezka_nolink: {
        ru: 'Не удалось извлечь ссылку Rezka',
        uk: 'Неможливо отримати посилання Rezka',
        en: 'Failed to fetch Rezka link',
        zh: '获取 Rezka 链接失败'
      },
      rezka_balanser: {
        ru: 'Источник Rezka',
      ...
      helper_rezka_file: {
        ru: 'Удерживайте клавишу "ОК" для вызова меню качества Rezka',
        uk: 'Утримуйте клавішу "ОК" для виклику меню якості Rezka',
        en: 'Hold the "OK" key to bring up Rezka quality menu',
        zh: '按住"确定"键调出 Rezka 质量菜单'
      },
      title_rezka_online: {
        ru: 'Rezka Онлайн (4K/1440p/1080p)',
        uk: 'Rezka Онлайн (4K/1440p/1080p)',
        en: 'Rezka Online (4K/1440p/1080p)',
        zh: 'Rezka 在线 (4K/1440p/1080p)'
      },
      rezka_voice_subscribe: {
        ru: 'Подписаться на перевод Rezka',
        uk: 'Підписатися на переклад Rezka',
        en: 'Subscribe to Rezka translation',
        zh: '订阅 Rezka 翻译'
      },
      rezka_voice_success: {
        ru: 'Вы успешно подписались на Rezka',
        uk: 'Ви успішно підписалися на Rezka',
        en: 'You have successfully subscribed to Rezka',
        zh: '您已成功订阅 Rezka'
      },
      rezka_voice_error: {
        ru: 'Возникла ошибка Rezka',
        uk: 'Виникла помилка Rezka',
        en: 'A Rezka error has occurred',
        zh: '发生了 Rezka 错误'
      },
      rezka_clear_all_marks: {
        ru: 'Очистить все метки Rezka',
        uk: 'Очистити всі мітки Rezka',
        en: 'Clear all Rezka labels',
        zh: '清除所有 Rezka 标签'
      },
      rezka_clear_all_timecodes: {
        ru: 'Очистить все тайм-коды Rezka',
        uk: 'Очистити всі тайм-коди Rezka',
        en: 'Clear all Rezka timecodes',
        zh: '清除所有 Rezka 时间代码'
      },
      rezka_change_balanser: {
        ru: 'Изменить источник Rezka',
        uk: 'Змінити джерело Rezka',
        en: 'Change Rezka source',
        zh: '更改 Rezka 来源'
      },
      rezka_balanser_dont_work: {
        ru: 'Поиск на Rezka ({balanser}) не дал результатов',
        uk: 'Пошук на Rezka ({balanser}) не дав результатів',
        en: 'Search on Rezka ({balanser}) did not return any results',
        zh: 'Rezka ({balanser}) 搜索未返回任何结果'
      },
      rezka_balanser_timeout: {
        ru: 'Источник Rezka будет переключен автоматически через <span class="timeout">10</span> секунд.',
        uk: 'Джерело Rezka буде автоматично переключено через <span class="timeout">10</span> секунд.',
        en: 'The Rezka source will be switched automatically after <span class="timeout">10</span> seconds.',
        zh: 'Rezka 来源将在<span class="timeout">10</span>秒内自动切换。'
      },
      rezka_does_not_answer_text: {
        ru: 'Поиск на Rezka ({balanser}) не дал результатов',
        uk: 'Пошук на Rezka ({balanser}) не дав результатів',
        en: 'Search on Rezka ({balanser}) did not return any results',
        zh: 'Rezka ({balanser}) 搜索未返回任何结果'
      ...
      rezka_quality_auto: {
        ru: 'Авто (лучшее доступное)',
        uk: 'Авто (найкраще доступне)',
    ...
    Lampa.Template.add('rezka_css', `
        ...
        .online-prestige__viewed{position:absolute;top:1em;left:1em;background:rgba(0,0,0,0.45);-webkit-border-radius:100%;border-radius:100%;padding:.25em;font-size:.76em}
        ...
        .online-empty-template>*{background:rgba(0,0,0,0.3);-webkit-border-radius:.3em;border-radius:.3em}
    ...
    function resetTemplates() {
      Lampa.Template.add('rezka_prestige_full', `<div class="online-prestige online-prestige--full selector">
      ...
      Lampa.Template.add('rezka_content_loading', `<div class="online-empty">
      ...
      Lampa.Template.add('rezka_does_not_answer', `<div class="online-empty">
            ...
            <div class="online-empty__buttons">
                ...
                <div class="online-empty__button selector change">#{rezka_change_balanser}</div>
            </div>
            <div class="online-empty__templates">
                <div class="online-empty-template">
                    <div class="online-empty-template__ico"></div>
      ...
      Lampa.Template.add('rezka_prestige_rate', `<div class="online-prestige-rate">
      ...
      Lampa.Template.add('rezka_prestige_folder', `<div class="online-prestige online-prestige--folder selector">
      ...
      Lampa.Template.add('rezka_prestige_watched', `<div class="online-prestige online-prestige-watched selector">
    ...
    var button = `<div class="full-start__button selector view--online rezka--button" data-subtitle="${manifst.name} v${manifst.version}">
    ...
    // Добавляем настройку качества Rezka в интерфейс Lampa
    function addQualitySettings() {
      if (Lampa.Settings) {
        Lampa.Settings.add({
          component: 'settings_main',
          panel: {
            name: 'rezka_quality_settings',
            title: 'Настройки Rezka',
            items: [
              {
                name: 'rezka_quality_priority',
                component: 'selector',
                title: 'Приоритет качества Rezka',
                values: [
                  { title: Lampa.Lang.translate('rezka_quality_auto'), value: 'auto' },
                  { title: Lampa.Lang.translate('rezka_quality_4k'), value: '2160p' },
                  { title: Lampa.Lang.translate('rezka_quality_1440p'), value: '1440p' },
                  { title: Lampa.Lang.translate('rezka_quality_1080p'), value: '1080p' },
                  { title: Lampa.Lang.translate('rezka_quality_720p'), value: '720p' }
                ],
                value: Lampa.Storage.get('rezka_quality_priority', 'auto'),
                onChange: function(value) {
                  Lampa.Storage.set('rezka_quality_priority', value);
                  preferredQuality = value;
                  Lampa.Noty.show('Настройки качества Rezka сохранены');
                }
              }
            ]
          }
