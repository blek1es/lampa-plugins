(function () {
    ...
    function startPlugin() {
        ...
        Lampa.Platform.tv() && Lampa.Listener.follow('app', function (e) {
            if (e.type == 'ready') {
                var ico = '<svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="18" cy="18" r="16" stroke="white" stroke-width="2"/><path d="M14 12L24 18L14 24V12Z" fill="white"/></svg>';

                Lampa.Template.add('menu_online_watch', '<li class="menu__item selector" data-action="online_watch"><div class="menu__ico">' + ico + '</div><div class="menu__text">Онлайн</div></li>');

                $('.menu .menu__list').eq(0).append(Lampa.Template.get('menu_online_watch', {}, true));

                $('body').on('click', '[data-action="online_watch"]', function () {
                    Lampa.Activity.push({
                        url: '',
        ...
        // Добавление кнопки на странице фильма
        Lampa.Listener.follow('full', function (e) {
            if (e.type === 'complite') {
                var button = $('<div class="full-start__button selector online-watch-button">\
                ...
                setTimeout(function () {
                    ...
                    // Пробуем разные варианты размещения
                    if (container.find('.full-start__buttons').length) {
                        container.find('.full-start__buttons').append(button);
                    } else if (container.find('.full-start').length) {
                        container.find('.full-start').append(button);
                    } else if (container.find('.full').length) {
                        container.find('.full').append(button);
                    }
                }, 100);
            }
        });
